...
class Learner:
    ...
    def run(self):
        ...
        model_params = self.model_mgr.pub_msg(Msg(type = MsgType.MODEL_MGR_GET_MODEL_PARAMS))
    ...
    def ray_run(self):
        while True:
            ...
            model_params = ray.get(self.model_mgr.pub_msg.remote(Msg(type = MsgType.MODEL_MGR_GET_MODEL_PARAMS)))
...
class LearnerMgr(Moduler):
    ...
    def init(self, *args, **kwargs):
        if self.use_ray:
            self.learners = [ray.remote(Learner).options(num_cpus=2).remote(cfg = self.cfg, id = i, policy = copy.deepcopy(self.policy), *args, **kwargs) for i in range(self.cfg.n_learners)]
        else:
